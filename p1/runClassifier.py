...
def learningCurveSet_knn(classifier, dataset):
    return learningCurve_knn(classifier, dataset.X, dataset.Y, dataset.Xte, dataset.Yte)

def learningCurve_knn(classifier, X, Y, Xtest, Ytest):
    """
    Generate a learning curve by repeatedly halving the amount of
    training data until none is left.
    ...
    for i in range(1, M+1):    # loop over "skip lengths"
        # select every 2^(M-i)th point
        ids = arange(0, N, 2**(M-i))
        if ids.size < classifier.opts['K']:
            print('Skip %d points...' % ids.size)
            continue
        Xtr = X[ids, :]
        Ytr = Y[ids]
        ...
        # train the classifier
        (trAcc, teAcc, Ypred) = trainTest(classifier, Xtr, Ytr, Xtest, Ytest)
        # (trAcc, teAcc, Ypred) = trainTestSet(classifier, Xtr, Ytr, Xtest, Ytest)
